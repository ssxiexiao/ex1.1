	...
	.controller('infoCtrl', function($scope, RecordService){
		...
		for(var i = 0; i < 2; i++){
			...
			for(var j = 0; j < state.length; j++){
				var tmp = state[j];
				$scope.info.choices[indice].push([tmp, angle]);
			}
		}
		...
		$scope.submit = function(){
			...
			for(var i = 0; i < $scope.info.choices.length; i+=2){
				...
				for(var j = 0; j < $scope.info.choices[i].length; j++){
					$scope.info.favourite[indice].push($scope.info.choices[i][j][2]);
				}
			}
			for(var i = 1; i < $scope.info.choices.length; i+=2){
				...
				for(var j = 0; j < $scope.info.choices[i].length; j++){
					$scope.info.accurate[indice].push($scope.info.choices[i][j][2]);
				}
			}
